...
export const newAccount = z.object({
  account: accountSchema.refine((s) => {
    if (!s.licenseExpiration) return false
    const exp = new Date(s.licenseExpiration);
    if (Number.isNaN(exp)) return false;
    return exp.getTime() > new Date().getTime()
  }, {
    message: "License expiration must be after the current date",
