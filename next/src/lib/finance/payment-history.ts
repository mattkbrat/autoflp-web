...
import type { AmortizationParams, AmortizationPayments } from "./amortization";
...
type FilteredHistory = ReturnType<typeof filterHistory>;
...
export const getPaymentSchedule = (
	...
	while (workingBalance > 0 && month < 50) {
		...
		const matchingHistory: FilteredHistory = !lastPmtDate
			? []
			: filterHistory(history, month, date);
		...
		schedule.push({
			...
			payments: matchingHistory.map((h) => {
				return Object.assign(h, {
					date: formatDate(h.date, "MMM dd ''yy"),
				});
			}),
			date,
			monthType,
