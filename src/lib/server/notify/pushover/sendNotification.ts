...
import { PUSHOVER_USER } from "$env/static/private";
import { pushoverToken } from "$lib/server/env";
import { type PushoverNotification, sounds } from "./constants";

export const sendNotification = async (p: PushoverNotification) => {
	if (!pushoverToken || !PUSHOVER_USER) {
		console.error("Missing pushover env");
		return;
	}
	...
	const params: PushoverNotification & {
		retry?: number;
		expire?: number;
	} = {
		...p,
