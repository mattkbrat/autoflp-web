...
$: termExceeded = scheduleRows.length > selected.term;
...
<h2 class="flex flex-col uppercase items-center">
  ...
  <span class=" tracking-tight">
    {fullNameFromPerson({ person: selected?.account.contact })}
  </span>
</h2>
<hr />
    ...
    <table class="w-full">
      ...
      <tbody class="font-mono text-right">
        ...
        {#each scheduleRows.filter((r) => {
          ...
          >
            <td>
              {formatDate(row.date, false, "MMM `yy")}
            </td>
            <td>
